  ...
  \x22cloudflare\x22: [
    {
      ...
      \x22subdomains\x22: [
        ...
        {
          \x22name\x22: \x22$sub_domain3\x22,
          \x22proxied\x22: $proxied3
        }
      ]
    }
  ],
...
~/.acme.sh/acme.sh --issue -d $domain3 --standalone
~/.acme.sh/acme.sh --installcert -d $domain3 --key-file /root/private3.key --fullchain-file /root/cert3.crt
echo "---------- 安装docker ----------"
...
echo "---------- 配置vmess监听端口 ----------"
read -p "请输入vmess所使用的port: " sport
read -p "请输入websocket路径: " spath
echo "---------- sing-box配置文件 ----------"
echo -e "{
    \x22inbounds\x22: [
        {
...
server {
        ...
        listen [::]:443 ssl;
        server_name $domain3;
        ssl_certificate       /root/cert3.crt;
        ssl_certificate_key   /root/private3.key;
        ssl_session_timeout 1d;
        ssl_session_cache shared:MozSSL:10m;
        ssl_session_tickets off;
        ssl_protocols         TLSv1.2 TLSv1.3;
