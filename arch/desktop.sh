...
if [ "$answer" = "y" ]; then
    ...
    if [ "$extra_programs" = "y" ]; then
        ...
        if [ "$arm_answer" = "y" ]; then
            ...
            sudo pacman -S --needed --noconfirm spectacle okular kate ktorrent kget yakuake kfind ksystemlog partitionmanager isoimagewriter filelight sweeper libreoffice-still
